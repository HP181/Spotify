...
const Player = () => {
    ...
    const setInitialVolume = async () => {
        try {
            const response = await fetch(`https://api.spotify.com/v1/me/player/volume?volume_percent=50`, {
                method: "PUT",
                headers: {
            ...
            if (!response.ok) {
                console.error("Failed to set initial volume");
            }
        } catch (error) {
    ...
    const CurrentlyPlayingTrack = async () => {
        try {
            ...
            // Only update the slider if the user is not interacting with it
    ...
    const handleSliderChangeEnd = async (value) => {
        if (PlayerData && PlayerData.item && PlayerData.item.duration_ms) {
            ...
            try {
                ...
                // Introduce a small delay before allowing automatic updates again
                setTimeout(() => {
                    setIsInteractingWithSlider(false);
                }, 500); // 500ms delay to prevent immediate override
            }
        }
    };
    ...
    const togglePlayPause = async () => {
        try {
            ...
            if (response.status === 200 || response.status === 204) {
                ...
                // CurrentlyPlayingTrack(); // Refresh track data to ensure slider is in sync
            } else {
    ...
    useEffect(() => {
        if (session) {
            ...
            // Polling for PlayerData refresh every 5 seconds
            const pollId = setInterval(() => {
                CurrentlyPlayingTrack();
            }, 5000);
            setPollingIntervalId(pollId);
        }
        return () => clearInterval(pollingIntervalId); // Clear polling interval on component unmount
    }, [session]);
